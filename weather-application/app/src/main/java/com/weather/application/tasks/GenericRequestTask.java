package com.weather.application.tasks;
...
import com.weather.application.activities.MainActivity;
...
public abstract class GenericRequestTask extends AsyncTask<String, String, TaskOutput> {
    ...
    protected final void handleTaskOutput(TaskOutput output) {
        switch (output.taskResult) {
            case SUCCESS: {
                ...
                if (ParseResult.CITY_NOT_FOUND.equals(parseResult)) {
                    Toast.makeText(this.context,"City Not found",Toast.LENGTH_LONG).show();
                    ...
                    Toast.makeText(this.context,"Failed response",Toast.LENGTH_LONG).show();
    ...
    private URL provideURL(String[] coords) throws UnsupportedEncodingException, MalformedURLException {
        ...
        String apiKey =  "a52a6c4ad68c9bc2";
        String urlSeparator = "/";

        StringBuilder urlBuilder = new StringBuilder("http://api.wunderground.com/api/");
        urlBuilder.append(apiKey);
        urlBuilder.append(urlSeparator);
        urlBuilder.append("conditions/q/");
        final String city = sp.getString("city", Constants.DEFAULT_CITY);
        urlBuilder.append(city);
        urlBuilder.append(urlSeparator);
        urlBuilder.append(city+".json");
        Log.v("urlBuilder",urlBuilder.toString());
        // urlBuilder.append(getAPIName()).append("?");
        /*if (coords.length == 2) {
            urlBuilder.append("lat=").append(coords[0]).append("&lon=").append(coords[1]);
        } else {
            final String city = sp.c, Constants.DEFAULT_CITY);
